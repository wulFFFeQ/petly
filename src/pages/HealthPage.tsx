...
export function HealthPage() {
  ...
  const { allowedPets, canWritePet } = useAuthorizedHealthScope()
  ...
  const anyWrite = allowedPets.some((p) => canWritePet(p.id))

  return (
    <div className="space-y-8" data-testid="health-page">
      <PageHeader
      ...
      {allowedPets.length === 0 ? (
        <p
          className="rounded-xl border border-[#E8E4DC] bg-[#FAF8F5] px-4 py-6 text-sm text-[#5A6660]"
          data-testid="health-page-denied"
        >
          Nemáte oprávnění zobrazit zdravotní údaje žádného mazlíčka.
        </p>
      ) : (
        <>
