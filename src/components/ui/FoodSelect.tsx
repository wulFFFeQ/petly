...
import { getFoodOptions } from '../../lib/petFoods'
...
export function FoodSelect({
  ...
  value,
  onChange,
  placeholder = 'Hledejte nebo vyberte krmivo…',
  className,
}: FoodSelectProps) {
  ...
  const options = getFoodOptions(petType)
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: MouseEvent) => {
      ...
      setOpen(false)
      // Keep custom typed value on outside click
  ...
  }, [open, listboxId, value, query, onChange])
  ...
  const dropdown = open
    ? createPortal(
        <div
          ...
          role="listbox"
          aria-label={label}
          className="fixed z-[70] max-h-56 overflow-y-auto overscroll-contain rounded-xl border border-[#E8E4DC] bg-white shadow-[0_12px_32px_rgba(25,30,27,0.12)]"
          style={{
            top: panelStyle.top,
        ...
        >
          <ul className="py-1.5">
            {filteredOptions.length === 0 ? (
              <li className="px-3 py-2.5 text-sm text-[#7D8B82]">
                Nic nenalezeno — po uložení se použije zadaný text
              </li>
            ) : (
                ...
                return (
                  <li key={option.value}>
                    <button
                      ...
                      onClick={() => selectFood(option.value)}
                      className={cn(
    ...
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      >
        <input
          ...
          onBlur={handleBlur}
          className="h-full min-w-0 flex-1 bg-transparent text-sm text-[#191E1B] outline-none placeholder:text-[#A3AEA7]"
        />
        <button
          type="button"
          tabIndex={-1}
          aria-label="Zobrazit krmiva"
          onMouseDown={(event) => event.preventDefault()}
          onClick={() => {
            setOpen((prev) => !prev)
            inputRef.current?.focus()
          }}
          className="shrink-0 text-[#7D8B82] hover:text-[#2C4A3E]"
        >
          <ChevronDown
            size={16}
      ...
      {dropdown}
      <span className="text-[11px] text-[#7D8B82]">
        Nabídka podle druhu mazlíčka — můžete i zapsat vlastní krmivo.
      </span>
    </div>
  )
}
