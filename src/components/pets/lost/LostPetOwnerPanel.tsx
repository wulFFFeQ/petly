import { Copy, ExternalLink, Map as MapIcon } from 'lucide-react'
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            {announcement.status === 'found' && (
              <p className="text-sm text-[#4A564F]">Děkujeme všem, kteří pomohli.</p>
            )}
            {announcement.status === 'lost' && (
              <p className="mt-2 text-sm text-[#4A564F]">
                Naposledy viděn: {announcement.lastSeen.publicLabel} ·{' '}
          ...
          {announcement.status === 'lost' && (
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                size="sm"
                className="gap-1.5"
                onClick={() => window.open(buildLostAnnouncementUrl(announcement.publicToken), '_blank')}
              >
            ...
            <div className="space-y-3">
              {[...reports].reverse().map((report) => (
                <LostPetReportDetail
                  ...
                  onFlag={() => setFlagReportId(report.id)}
                  onContactFinder={
                    report.type === 'found'
                      ? () => {
                          ...
                          if (conv) {
                            ...
                            showToast(
                              'Konverzace není dostupná',
                              'Thread s nálezcem se nepodařilo najít.',
                              'info',
                            )
                          }
  ...
  return (
    <Card variant="elevated" padding="none" className="overflow-hidden">
      ...
      <p className="border-t border-[#E8E4DC] bg-[#FAF8F5] px-3 py-2 text-[11px] leading-relaxed text-[#7D8B82]">
        ...
        možné stopy pohybu podle nahlášených spatření.
      </p>
    </Card>
  )
}
