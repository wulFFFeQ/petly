/**
 * Server-side authorize equivalent of src/lib/security/authorize.ts (K47).
 * Fail closed. Role ≠ permission ≠ grant. Never trust body actor/permission claims.
 */
...
export type AuthorizationDecision =
  | {
      allowed: true
      reason: string
      path: 'owner' | 'household' | 'professional' | 'organization' | 'booking' | 'payment' | 'messaging' | 'public'
      grantId?: string
      permission?: string
    }
  | {
...
function grantActive(status: string, expiresAt: string | null, revokedAt: string | null, now: number): boolean {
...
export function authorizePetAction(input: {
  ...
  if (action === 'clinical.sign' || action === 'clinical.finalize' || action === 'clinical.admin' || action === 'clinical.export') {
  ...
  if (mode === 'organization') {
    for (const g of input.orgPetGrants) {
      ...
      // Membership alone is never enough — already filtered by assignment/eligibility in loader
  ...
  // personal / household path — no UNION with pro/org
...
/** Messaging: participant list is the only ACL. Missing list ⇒ deny. */
...
/** Reject forged client identity claims. */
